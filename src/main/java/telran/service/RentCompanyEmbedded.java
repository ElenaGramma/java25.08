...
import static telran.service.RentCompanyLocks.*;
...
public class RentCompanyEmbedded extends AbstractRentCompany implements Persistable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 1826791567338318599L;
	// 1
	...
	private HashMap<String, List<Car>> modelCars = new HashMap<String, List<Car>>();
	...
	public CarsReturnCode rentCar(String regNumber, long licenseId, LocalDate rentDate, int rentDays) {
		...
		try {
			...
			if (car.isInUse())
				return CAR_IN_USE;
			RentRecord recort = new RentRecord(regNumber, licenseId, rentDate, rentDays);
			addToRecords(recort);
			addToCarRecords(recort);
			addToDriverRecords(recort);
			car.setInUse(true);
			return OK;
		} finally {
			lockRentCar(false);
	...
	private void addToDriverRecords(RentRecord recort) {
		long key = recort.getLicenseId();
		List<RentRecord> list = driverRecords.getOrDefault(key, new ArrayList<RentRecord>());
		list.add(recort);
		driverRecords.putIfAbsent(key, list);

	}

	private void addToCarRecords(RentRecord recort) {
		String key = recort.getRegNumber();
		List<RentRecord> list = records.getOrDefault(key, new ArrayList<RentRecord>());
		list.add(recort);
		carRecords.putIfAbsent(key, list);

	}

	private void addToRecords(RentRecord recort) {
		long key = recort.getRentDays();
		List<RentRecord> list = records.getOrDefault(key, new ArrayList<RentRecord>());
		list.add(recort);
		driverRecords.putIfAbsent(key, list);
	...
	@Override
	public Set<Car> getCarsByDriver(long lolicenseId) {
		lockGetCarsByDriver(true);
		try {
			List<RentRecord> list = driverRecords.getOrDefault(lolicenseId, new ArrayList<>());
	...
	public List<Car> getCarsByModel(String modelName) {
		...
		try {
			List<Car> list = modelCars.getOrDefault(modelName, new ArrayList<Car>());
	...
	public RemovedCarData removeCar(String regNumber) {
		...
		try {
			...
			return car.isInUse() ? null : actuaCarRemove(car);
		} finally {
			lockRemoveCar(false);
		}
	}

	private RemovedCarData actuaCarRemove(Car car) {
	...
	private void removeFromDriverRecords(List<RentRecord> list) {
		list.forEach(rr -> {
			...
			List<RentRecord> res = driverRecords.get(id);
			res.remove(rr);
			if (res.isEmpty()) {
	...
	public RemovedCarData returnCar(String regNumber, long licenseId, LocalDate returnDate, int damagesPercent,
		...
		lockReturnCar(true);
		try {
			if (regNumber == null || licenseId < 1 || returnDate == null || damagesPercent < 0)
				return null;
			...
			return car.isFlagRemoved() || damagesPercent >= REMOVE_THRESHOLD ? actuaCarRemove(car) : null;
		} finally {
			lockReturnCar(false);
		}
	}
	...
	private int getDelay(RentRecord record) {
		long realDays = ChronoUnit.DAYS.between(record.getRentDate(), record.getRentDate());
